...
export function PaintingsExperience() {
  ...
  const expandedId = getExpandedCategoryId(fakeData.paintingCategories, categoryId);
  const selectedIds = useMemo(
    () => categoryId ? new Set(getCategoryAndDescendantIds(fakeData.paintingCategories, categoryId)) : null,
    [categoryId],
  );
  const paintings = useMemo(
    () => selectedIds
      ? fakeData.artworks.filter(artwork => artwork.paintingCategoryIds.some(id => selectedIds.has(id)))
      : fakeData.artworks,
    [selectedIds],
  );
  ...
  return (
    <main className="paintings-experience" data-series-expanded={expandedId === 'series'} id="main-content">
      <PaintingCategoryFilter categories={fakeData.paintingCategories} value={categoryId} onChange={setCategoryId} />
