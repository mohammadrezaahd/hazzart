...
- `components/Slider`: the slider shared by Paintings and Projects.
  - `useSliderScroll.ts`: the scroll engine. Pointer drags and touch flicks stay native; wheel, keyboard and the range control are damped frame by frame. It measures the track once per layout change and then keeps the 60fps loop out of React, except for the active slide index. A bounded track reserves trailing room so its last slide can also reach the left edge.
...
- The page is a plain flex column — header space, story, strip, range control — so the range control always stays inside the viewport.
